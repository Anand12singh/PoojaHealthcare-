...
$.validator.addMethod("imageSize", function (value, element, param) {
    ...
    return isImageValid;
}, "Icon must be a PNG, SVG image with a resolution of 512x512 pixels.");
...
var validator = $("#addEditForm").validate({
    rules: {
        icon: {
            ...
            imageSize: [512, 512] // Custom validation rule (handles both size & format)
        }
    },
    messages: {
      icon: {
    ...
    submitHandler: function (form) {
        if (!isImageValid) {
            validator.element("#icon"); // Ensure error message shows
...
function resetForm() {
  $("#addEditForm")[0].reset();
  $('#cat_id').val('');
  $("#mediaPreview").attr('src','https://placehold.co/512x512')
  $(".error").text("");
  return false;
}
...
//         const errorMessage = document.querySelector('.icon-error');
...
//                 errorMessage.textContent = 'Image dimensions must be 512*512.';
//                 return false;
//             }
